...
class Auth
{
    ...
    */
    ...
    public function __construct(\PDO $dbh, $config)
    {
        ...
        // Include password compatability layer if required :
            ...
            if ($validateUsername['error'] == 1) {
                ...
                if($user) {
                    if (password_verify($password, $user['password'])) {
                        if ($user['isactive'] == 1 && $user['isadmin']) {
    ...
    */
    ...
    public function getUser($uid)
    {
        ...
        $query = $this->dbh->prepare("SELECT username, password, email, salt, isactive, isadmin FROM {$this->config->table_users} WHERE id = ?");
        $query->execute(array($uid));
    ...
    */
    ...
    private function addRequest($uid, $email, $type)
    {
        ...
        if($query->execute(array($uid, $key, $expire, $type))) {
            if($type == "activation") {
                $message = "Account activation required : <strong><a href=\"{$this->config->site_url}/auth/#/activate/{$key}\">Activate my account</a></strong>";

                $headers  = 'MIME-Version: 1.0' . "\r\n";
                $headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
                $headers .= "From: {$this->config->site_email}" . "\r\n";

                if(@mail($email, "{$this->config->site_name} - Account Activation", $message, $headers)) {
                    $return['error'] = 0;
                    return $return;
                } else {
                    $return['message'] = "system_error";
                ...
                $message = "Password reset request : <strong><a href=\"{$this->config->site_url}/auth/#/reset/{$key}\">Reset my password</a></strong>";

                $headers  = 'MIME-Version: 1.0' . "\r\n";
                $headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
                $headers .= "From: {$this->config->site_email}" . "\r\n";

                if(@mail($email, "{$this->config->site_name} - Password Reset Request", $message, $headers)) {
                    $return['error'] = 0;
                    return $return;
                } else {
                    $return['message'] = "system_error";
